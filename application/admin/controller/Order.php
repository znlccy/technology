...
class Order extends BasisController {
    ...
    public function save() {
        ...
        /* 返回结果 */
        if (empty($id)) {
            $order_id = $this->order_model->insertGetId($validate_data);
            $order = $this->order_model->where('id', $order_id)->field('order_id')->find();
        } else {
            $order = $this->order_model->save($validate_data, ['id' => $id]);
        }

        if ($order) {
            return $this->return_message(Code::SUCCESS, '数据操作成功', $order);
        } else {
    ...
    public function supplement() {
        ...
        if ($charge) {
            ...
            return $this->return_message(Code::SUCCESS, '补单成功');
        } else {
            return $this->return_message(Code::FAILURE, '补单失败');
        }
